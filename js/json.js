...
function json_encode (mixed_val) {
    ...
    try {
        if (typeof json === 'object' && typeof json.stringify === 'function') {            retVal = json.stringify(mixed_val); // Errors will not be caught here if our own equivalent to resource
        ...
        var str = function (key, holder) {
            var gap = '';
            var indent = '    ';            var i = 0;          // The loop counter.
            var k = '';          // The member key.
            var v = '';          // The member value.
            var length = 0;
            var mind = gap;            var partial = [];
            ...
            switch (typeof value) {
                ...
                case 'object':
                    ...
                    // and wrap them in braces.                    v = partial.length === 0 ? '{}' :
                            ...
                            mind + '}' : '{' + partial.join(',') + '}';
                    gap = mind;
                    return v;                case 'undefined': // Fall-through
        ...
        return str('', {            '': value
                            ...
                            // (i.e., when the JSON global is not available and there is an error)        if (!(err instanceof SyntaxError)) {
            throw new Error('Unexpected error type in json_encode()');
        }
        this.php_js = this.php_js || {};
        this.php_js.last_error_json = 4; // usable by json_last_error()        return null;
}
